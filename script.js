...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const savedHTML = localStorage.getItem('la-morning-ai-handbook-content');
    if (savedHTML) {
    ...
    editToggle.addEventListener('click', () => {
        ...
        if (isEditing) {
            ...
            localStorage.setItem('la-morning-ai-handbook-content', contentArea.innerHTML);
    ...
    if (resetTemplateBtn) {
        resetTemplateBtn.addEventListener('click', () => {
            if (confirm('確定要清除所有的自訂修改與圖片，恢復為原始的教材內容嗎？（此操作無法復原）')) {
                localStorage.removeItem('la-morning-ai-handbook-content');
    ...
    confirmImgBtn.addEventListener('click', () => {
        ...
        // 圖片插入後立即發送通知提示
        showToast('📷 圖片已成功插入！按「結束編輯」即可完成儲存。');
    });
